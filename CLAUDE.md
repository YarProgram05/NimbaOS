...
> **Сессия 2026-03-25 — UX-улучшения и баг-фиксы (пост-Фаза 4)**
> Подробности в разделе «Принятые решения» — пп. 25–31.
...
15. **Prices API v2 возвращает рубли, не копейки** — `/api/v2/list/goods/filter` (`prices` домен) отдаёт цены уже в рублях. Деление на 100 не нужно. (Цены из cards API `/content/v2/get/cards/list` — в копейках, делятся на 100 при сохранении, но они используются только как fallback и не отображаются напрямую.)

16. **`ProductSize.spp` хранит `discountedPrice` из prices API** — поле `spp` (скидка постоянного покупателя) переиспользуется для хранения `discountedPrice` из `/api/v2/list/goods/filter` — это фактическая цена продавца (цена со скидкой). Настоящий SPP-процент будет приходить из отчётов реализации в Фазе 5. В Фазе 3 SPP% вычисляется обратно: `round((1 - spp/price) * 100)`.

17. **`ProductSize.price` = базовая цена продавца (до скидки)** — поле `price` хранит базовую (зачёркнутую) цену. `ProductRow.price` в Server Action = `price * (1 - discount/100)` — цена со скидкой, которую видит покупатель. `ProductRow.basePrice` — сама базовая цена для редактирования.

18. **Редактирование цены через popover + WB API** — `updateProductPriceAction` отправляет задачу `POST /api/v2/upload/task` (цены домен), затем делает оптимистичное обновление в БД. WB обрабатывает задачу асинхронно за несколько секунд.
...
25. **Sidebar NavLinks включают `?account=id`** — `NavContent` читает `selectedId` из `useAccount()` и добавляет `?account=selectedId` ко всем ссылкам навигации. Это фиксирует баг: при переходе на /cards или /references из страниц без `?account=` (отчёты, настройки и т.д.) серверный компонент видит правильный кабинет, а не первый активный.

26. **Колонка «Наименование» убрана из /cards** — лишняя колонка удалена для экономии пространства. Шрифт таблицы поднят до `text-base`. Осталось 5 колонок: Арт. WB, Арт. поставщика, Категория, Бренд, Цена.

27. **Арт. WB — ссылка с hover-превью фото** — nmId стал кликабельной `<a>` на `wildberries.ru/catalog/{nmId}/detail.aspx`. При наведении — Tooltip с фото товара (из `Product.photoUrl`). Tooltip рендерится в `columns.tsx` через `TooltipProvider`.

28. **Цена в /cards — цветной прямоугольник + СПП в ячейке** — блок цены оформлен в светло-голубой прямоугольник (`bg-sky-50 border-sky-200 rounded-lg`). Прямо в ячейке (не только в tooltip) отображается «с СПП (30%) 1300 ₽». Прямоугольник выровнен по левому краю ячейки (`inline-flex`).

29. **Пагинация /cards — 100 записей/стр, расположена по центру** — дефолт `pageSize` изменён с 50 на 100. Пагинация перемещена в центр (`justify-center`).

30. **VIEWER не может управлять кабинетами WB** — `/settings` передаёт роль в `AccountsSection`. При `isReadOnly=true` (роль VIEWER) скрыты: кнопка «Добавить кабинет», кнопка удаления, редактирование налоговой ставки.

31. **Кнопка RefreshCw в попапе цены — активная** — иконка `RefreshCw` рядом с «Базовая цена» стала кликабельной кнопкой. Вызывает `refreshProductPriceAction(wbAccountId, nmId)`, который запрашивает `/api/v2/list/goods/filter?filterNmID=...`, обновляет БД и возвращает свежие данные в поля редактирования. Добавлена `fetchPricesByNmId` в `src/lib/wb-api/products.ts`.

32. **VendorCombobox — скролл колёсиком работает внутри диалогов** — добавлен `onWheel={(e) => e.stopPropagation()}` на контейнер списка. Фиксирует проблему: Radix Dialog блокировал wheel-события до того, как они достигали `overflow-y: auto` div.

33. **VendorCombobox в самовыкупах и внешней рекламе** — ранее поля артикула в `self-purchase-tab.tsx` и `external-ad-tab.tsx` были простыми `<Input>`. Заменены на `VendorCombobox` (с поиском, как в себестоимости и переименованиях). При редактировании — статичный текст (паттерн как в cost-price).

---

## О проекте

Ты разрабатываешь **NimbaOS** — закрытую веб-платформу для оцифровки кабинетов продавца на Wildberries. Полная спецификация проекта находится в файле `SPECIFICATION.md` — прочитай его перед началом любой работы.
