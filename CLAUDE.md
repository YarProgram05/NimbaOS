...
Последнее обновление: **2026-03-27** | Следующая задача: **Фаза 6 (план продаж)**
...
- `Итого` теперь считается как сумма значений строк отчёта, а не отдельной формулой поверх сырых данных
- В шапку отчёта добавлен аккуратный счётчик количества артикулов
- Исправлены сломанные строковые сравнения WB-типов операций, из-за которых часть колонок могла уходить в нули
...
- Добить точечные расхождения по редким кейсам WB, если всплывут новые нестандартные операции
- При желании вынести отдельно “нераспределённые” расходы, если понадобится показывать их вне строк артикулов
...
- Требуется дальнейшая ручная сверка на живых кабинетах WB по редким операциям, которые слабо документированы в API
