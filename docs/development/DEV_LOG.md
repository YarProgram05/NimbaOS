...
The home page now fills the available main height and assigns all remaining space to the central chart/focus row. The compact chart and its three distribution cards use that row's height instead of a fixed 240/260 px value. A height breakpoint below 1000 px switches gaps and panel padding from spacious to compact without hiding any metric or bottom-card detail; fullscreen restores the spacious density and expands the graph. The separate full analytics chart remains 432 px.

An initial responsive pass incorrectly kept the forced full-height flex allocation below the desktop breakpoint. At 990x909, the middle panels and compact chart changed to vertical layouts but were still compressed into the desktop remainder, causing visible overlay. The final layout applies elastic height only from `xl`; narrower layouts use a 260 px chart and natural stacked panel heights with normal vertical scrolling.

Authenticated browser QA measured exact 1920x952 normal-window, 1920x1001 breakpoint and 1920x1080 fullscreen viewports. All three reported equal main `scrollHeight` and `clientHeight`, no horizontal overflow and no console errors; fullscreen reported effectively zero unused page space above the normal bottom padding. At 990x909, geometry checks reported zero overlap between all adjacent top-level sections, middle panels and chart/distribution parts, with no horizontal overflow. Type-check, lint and all 60 tests passed; the two existing unrelated `<img>` lint warnings remain.

## 2026-09-01 - Flexible grouped sync schedule settings

Implemented the approved catalog-and-drawer UX for `/sync`. The schedule card no longer contains a minimum-width table or exposes eleven editable rows simultaneously. A declarative sync catalog groups tasks into five business categories and provides title, explanation, snapshot/rolling-depth behavior and recommended defaults. Cards show the persisted state, readable schedule, depth and next run; the selected task opens in a responsive right-side drawer that becomes full width on narrow screens.

Extracted `FlexibleScheduleEditor` and generic schedule calculation utilities so automations and syncs share exact-time, interval, weekday, validation, formatting and next-run logic. Automations retain daily/weekly/every-N-weeks/monthly behavior. Syncs expose daily or selected weekdays, and every sync kind offers both one/several exact times and a bounded interval window under the same global 5-720 minute / 288 runs-per-day validation. Snapshot jobs omit only the unrelated rolling-depth field. The drawer adds next-three-run preview, overlapping-time warnings, explicit enable/save controls and per-kind recommended reset.

Added nullable JSON `SyncScheduleSetting.schedule` with committed migration `20260901143000_flexible_sync_schedules`. Existing rows with only `timeOfDay`/`intervalMinutes` are normalized without an automatic rewrite; the JSON form is stored on the next normal save while compatibility columns keep the primary time/interval. Applying a schedule removes the old/current prefix and registers one BullMQ scheduler per effective clock time. Payload fingerprint and selected time protect the worker from stale jobs after replacement, including queued legacy jobs once JSON is saved.

The migration target was read-only-verified as local PostgreSQL `wb_cabinet` at `localhost:5432` before applying it locally. No schedule form was saved, no scheduler was reapplied, and no sync, queue worker, WB API or production state changed. Authenticated browser QA verified 11 configuration cards, zero horizontal overflow at 908, 1074 and 1280 px, the bounded interval option and its start/end/minutes fields on ordinary `Карточки` as well as FBS controls, recommended reset to 08:00–23:55/every 5 minutes/1 day, hidden rolling depth for cards, multiple exact times and clean cancellation. Final page console contained no warnings/errors. Verification passed 60 tests, TypeScript, lint and production build; lint/build retain only two unrelated pre-existing `<img>` warnings. Local dev was restarted after the build and `/api/health` returned 200.

## 2026-09-01 - Sync schedule settings UX audit

Reviewed the live authenticated `/sync` page, the existing automation schedule editor, `SyncScheduleSetting`, BullMQ schedule application and the worker's rolling-period behavior without changing settings or code. The current schedule table forces an 820 px minimum width inside the already narrowed dashboard content. At the normal 1074 px browser viewport, the table's save column is clipped to the right, confirming that the primary configuration experience is not adaptive.

The audit also found a domain mismatch hidden by the uniform columns. Cards, campaign metadata and current stock are snapshot jobs with no meaningful day-depth setting; reports, sales plan, ad statistics, reviews/questions and two FBS jobs consume rolling days; the three FBS schedules are frequent intervals and currently run around the clock once enabled. The scheduler stores only one daily time or one global interval per job, whereas automations already support multiple exact times, weekdays and interval windows. Reusing that rule engine is appropriate, but copying its complete editor into every sync row is not.

Prepared three UX directions for owner review: a grouped compact catalog plus one right-side task editor (recommended), grouped inline accordions, and a visual daily timeline. The recommended model keeps overview, status, next run and schedule summaries visible, renders only capabilities relevant to the selected sync kind, labels rolling depth contextually, previews upcoming runs and warns about collisions/rate-limit pressure before an explicit save reapplies BullMQ. No sync, schedule, queue, worker, WB API, database or production mutation occurred.

## 2026-09-01 - Shared automation table settings template

Implemented the owner-selected hybrid of the unified form and persistent validation summary. Both automation detail pages now use one responsive `GoogleSheetTemplateEditor` for connection, workbook scan, cabinet mapping and local validation. FBS role definitions are declarative metadata, and saved configuration uses `sheetTabs: role -> actual tab`; the workflow runtime resolves stable roles rather than individual `operationsSheetName`, `summarySheetName` and similar fields. A centralized compatibility adapter reads the legacy JSON shape without requiring a Prisma migration or automatic rewrite; the new shape is written only by the normal user save path.

The read-only inspection action returns workbook title, timezone, dimensions and actual tab names through the existing service-account connector. The editor auto-matches known defaults, supplies discovered names without removing manual entry, marks missing/renamed tabs, rejects duplicate role or cabinet mappings, validates enabled cabinets and hides FBS technical keys in an expandable advanced section. At the initial 1074 px authenticated browser viewport, the first two-column breakpoint squeezed role descriptions; moving the validation sidebar to the wider `xl` breakpoint restored full-width rows and retained zero horizontal overflow.

After owner review, the connection result was extended into an explicit workbook-tab registry. It shows found/used/free totals and every detected tab with `Используется` or `Не используется`; unused tabs are informational rather than validation errors and remain available in the existing mapping inputs. Live read-only QA reported FBS `8 / 5 / 3` and morning report `3 / 3 / 0`, with no horizontal overflow or browser console errors.

Authenticated QA scanned the live FBS workbook (8 tabs) and morning-report workbook (3 tabs), recognized all configured names, showed the successful validation state, disabled save for a deliberately missing tab, exposed advanced technical keys, and produced no console errors. The test change was discarded by reload; no setting was saved and no automation, schedule, worker or Google Sheet write was triggered. Verification passed 57 tests, TypeScript, lint and production build; only the two existing unrelated `<img>` warnings remain. The local dev server was restarted after the build and `/api/health` returned HTTP 200.

## 2026-09-01 - Unified adaptive expandable run-history cells

Standardized the complete histories on `/automations` and `/sync` with one shared presentation contract. Both history tables now use the same page content width and a responsive 100% fixed table layout with percentage-based column sizing. Every informational cell has a 64 px collapsed height, a two-line clamp, consistent wrapping for long tokens and a click/keyboard toggle that reveals the complete stored value; a second click collapses it again. The independent sync delete control was intentionally kept outside the disclosure behavior.

The initial 1440 px draft was rejected during owner review because it introduced a horizontal scrollbar inside the normal workspace. Replaced it with percentage columns and responsive header spacing. Authenticated browser QA at a 1394 px viewport confirmed both history tables render at 1036 px, exactly matching their wrapper client/scroll width with no page or table horizontal overflow. All ten headers remain readable, and all normal rows remain 65 px including the border. Long automation and sync API errors still disclose their complete text. The tables, filters, sorting, pagination and delete control remained intact.

Verification passed `npm run type-check`, `npm run lint`, all 52 tests and `npm run build`; lint/build retain only the two existing unrelated `<img>` warnings. The local dev server was restarted after the build for authenticated QA. No automation run, sync job, schedule save, worker, WB API call or production mutation occurred.
