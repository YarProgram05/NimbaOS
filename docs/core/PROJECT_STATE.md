...
Update 2026-09-01: the main dashboard now adapts to normal Full HD Chrome, browser fullscreen and narrower layouts at 100% zoom. At `xl` width the page fills available height and the central chart/focus row elastically consumes the remainder; below `xl`, panels use natural stacked height and normal vertical scrolling so graphs, distributions and cards cannot overlap. A 1000 px height breakpoint changes only spacing/padding density without hiding data. Exact 1920x952, 1920x1001 and 1920x1080 QA found no vertical/horizontal overflow; 990x909 geometry checks found zero panel overlap and no horizontal overflow. Console remained clean. The separate full analytics chart is unchanged.

Update 2026-09-01: `/sync` schedule settings now use a responsive grouped catalog with one right-side editor instead of a wide eleven-row form. Syncs and automations share one flexible time-rule engine; all sync kinds provide the same exact-time and bounded-interval modes with global validation, while metadata only hides rolling depth for snapshot jobs and supplies recommendations. `SyncScheduleSetting.schedule` stores the new JSON value with legacy time/interval compatibility; stale scheduled payloads are fingerprint-gated. Migration `20260901143000_flexible_sync_schedules` is applied only to the local development database and awaits normal production deployment. No schedule was saved or reapplied during QA.

Update 2026-09-01: automation Google Sheet settings now use one shared responsive template editor. Stable workflow roles map to editable actual tab names through generic `sheetTabs` configuration; FBS legacy individual fields remain readable until the next normal save. Both current workflows can scan workbook tabs read-only, list every detected tab as used or unused with found/used/free totals, validate renamed/missing/duplicate mappings, configure cabinets in the same layout and keep technical keys under advanced settings. Local authenticated QA verified FBS at 8 found / 5 used / 3 free and the morning workbook at 3 / 3 / 0 without saving or running either automation.
...
- Automations: catalog at `/automations`, per-workflow settings at `/automations/[kind]`, flexible JSON-backed schedules, named/filterable `AutomationRun` history, separate automation queue, and a shared schema-driven Google Sheet/cabinet template editor with workbook scan and role-to-tab validation. Current workflows are `Утренний отчет WB` and `Заказы FBS → таблица учета`; the FBS workflow upserts stable lifecycle-event keys, verifies daily reconciliation and supports explicit physical-product aliases.
