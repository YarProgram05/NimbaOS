...
Заказы и отмены считаются по `fbs_orders.createdAtWb` выбранного периода; отменённый заказ считается один раз по терминальному supplier/WB-статусу. Выкупы, возвраты, выручка и сумма к перечислению считаются по финансовым операциям выбранного периода. Финансовая строка относится к FBS через `realization_reports.orderId = fbs_orders.externalOrderId`; прямой `deliveryMethod=FBS` — только fallback, потому что WB часто заполняет его на логистической строке, а не на строке продажи.

На уровне артикула прямой `ОП FBS = К перечислению - Логистика - Хранение - Приёмка - Доплаты - Штрафы - Удержания - Себестоимость - Налог`. `Маржинальность FBS = ОП FBS / Выручка FBS × 100%` при положительной выручке, иначе `0%`. Это contribution-style показатель конкретного способа доставки: общие рекламные и внешние рекламные расходы не включаются, пока нет проверяемого правила распределения между FBS и FBO. Поэтому его нельзя напрямую сравнивать с полной ОП финансового отчёта без учёта этого ограничения.

`Рентабельность FBS = ОП FBS / Совокупные затраты FBS × 100%`, где совокупные затраты в текущей модели равны `Выручка FBS - ОП FBS`; при неположительных затратах показатель равен `0%`. `Процент выкупа FBS = max(0, Выкупы - Возвраты) / (Выкупы + Отмены) × 100%`; незавершённые заказы в знаменатель не входят. На уровне артикула применяются те же формулы по его строкам.

Формулы: `Выкупы` = сумма quantity строк `Продажа`; `Возвраты` = сумма quantity строк `Возврат`; `Выручка` = продажи с СПП минус возвраты с СПП; `К перечислению` = `ppvzForPay` продаж минус `ppvzForPay` возвратов. Нельзя подменять финансовый выкуп статусом сборочного задания.
