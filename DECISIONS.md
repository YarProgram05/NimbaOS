...
**Decision 59: Workflow schedules live in existing config JSON and use a registry**
Automation identity remains the persisted enum `AutomationRun.kind` / `AutomationWorkflowSetting.kind`, while user-facing names and descriptions live in the application registry. Flexible cadence is stored as `config.schedule` in the existing workflow row and the legacy `timeOfDay` keeps the first effective time for compatibility; adding schedule variants therefore does not require a database migration. BullMQ receives one scheduler per effective time. Anchored every-N-weeks recurrence is enforced by a worker-side Moscow-calendar gate because cron alone cannot represent that cycle reliably.

**Decision 60: FBS movement sheet is a DB-first lifecycle-event projection**
The FBS accounting workflow reads only locally synchronized NimbaOS data and projects immutable technical event keys into the employee-readable Google Sheet. Orders use account + externalOrderId, cancellations use a separate pre-handoff event key, and accepted returns require an explicit `RETURN_RECEIVED` inventory movement. Existing keys are updated in place and missing keys are inserted; duplicates, unknown products, missing sync coverage, broken headers/formulas or reconciliation mismatches fail the run. Physical-product aliases are keyed by stable account/nmId/chrtId tuples: `nimba:297175085:452136209` is canonically `туника леопард/пятна` despite the duplicate WB listing name `парео леопард/пятна`, and `nimba:232092449:366203604` is canonically `туника синие волны` despite the duplicate listing name `парео синяя полоска`.

**Decision 61: WB-confirmed FBS stock and the local movement ledger are separate signals**
The Sheet columns named `Остаток WB` are authoritative current sellable quantities from the locally synchronized `fbs_assortment_items.wbStock` snapshot. They are not reconstructed from initial balance, orders or movements. For the current hourly FBS sync cadence, the workflow requires a snapshot no older than 75 minutes, upserts it by account/nmId/chrtId and fails if Summary does not exactly match NimbaOS. A local physical balance below WB indicates a likely omitted local replenishment and produces the non-blocking human-readable warning `ВНЕСТИ ПОПОЛНЕНИЕ +N`; it must not make a valid WB snapshot load fail.
